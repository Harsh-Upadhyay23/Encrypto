...
if (form.site.length < 3 || form.username.length < 3 || form.password.length < 3) {
  toast.error("Credentials are too short");
  return;
}
  ...
  const editPassword = (id) => {
    let newpass=passwordArray.filter(i=>i.id==id)[0];
    setForm(newpass);
     const updatedArray = passwordArray.filter(item => item.id !== id);
        setPasswordArray(updatedArray);
 ...
 return (
  <>
    ...
    <div className="flex flex-col items-center justify-start px-4 md:px-6 lg:px-8 m-auto w-full md:w-3/4 lg:w-1/2">
      {/* Form Card */}
      <div className="w-full p-4 sm:p-6 bg-white rounded-2xl shadow-xl border border-gray-200">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <button
            ...
            className="flex items-center justify-center gap-1 rounded-md bg-green-600 px-4 py-2 text-white font-medium hover:bg-green-500"
          >
            <lord-icon
      ...
      <div className="w-full mt-6">
        <h2 className="font-bold text-xl sm:text-2xl py-4 text-center text-gray-800">
          Your Passwords
        </h2>
        ...
        {passwordArray.length === 0 ? (
          ...
          <div className="overflow-x-auto shadow-lg rounded-lg mb-5">
              ...
              <tbody className="bg-green-50 text-gray-800">
                {passwordArray.map((item, index) => (
                  <tr key={index} className="border-b">
                    <td className="py-2 px-2 sm:px-4 text-center break-words">
                      {item.site}
                    </td>
                    <td className="py-2 px-2 sm:px-4 flex items-center justify-center gap-1 break-words">
                      {item.username}
                      <span
                        onClick={() => {
                      ...
                      >
                        <img width={18} src="/public/icons/3719119.png" alt="" />
                      </span>
                    </td>
                    ...
                    <td className="py-2 px-2 sm:px-4 text-center flex justify-center gap-2">
                      <span
